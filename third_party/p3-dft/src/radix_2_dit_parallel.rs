    ...
    F: TwoAdicField + Ord,
{
    fn get_or_compute_twiddles(&self, log_h: usize) -> Arc<VectorPair<F>> {
        ...
        // BlackSilk patch (third_party/README.md, AUDIT.md ZK-F21): compute the
        // twiddles *before* taking the write lock. `collect_n` is parallel for
        // large sizes, and running rayon work under this spin lock let a
        // thread steal a DFT task that then spun on the lock held further up
        // its own stack. Two threads may now compute the same (deterministic)
        // table on a miss; the first insert wins.
        let computed = {
