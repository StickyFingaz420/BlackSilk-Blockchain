...
use decentralized_storage::DecentralizedStorage;
use models::*;
...
async fn index(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let featured_products = match state.storage.query_data(|data| {
        matches!(data, decentralized_storage::MarketplaceData::ProductListing(product) if product.is_active)
    }).await {
        Ok(products) => {
            let mut featured = Vec::new();
            for data in products.into_iter().take(12) {
                if let decentralized_storage::MarketplaceData::ProductListing(product) = data {
