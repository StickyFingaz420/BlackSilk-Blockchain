...
| Z7 | Zero knowledge of the proofs as configured: the hiding PCS (4 random codewords, 4 salt elements, one random row per trace row) hides the committed columns at every opened point, and nothing else published depends on the witness | Every private property of PX | **Partly addressed, OPEN.** The LogUp terminals leaked (ZK-F29) and are now blinded (terminal-blinding.md; R12). Small tables now have 256 or more random rows (ZK-F30). **Not established by the project:** that Plonky3's hiding PCS is zero knowledge as configured (U, O). In particular, the FRI commit-phase openings are masked with only 4 base-field random codewords, a 4-dimensional subspace of the extension (internal review round 2, Z2). Every privacy claim that depends on Z7 carries this caveat |
...
| Z13 | 256 random rows per table exceed the openings per column (108 queries plus 2 out-of-domain points) enough for the hiding PCS to hide the committed values | ZK-F30; the blinding values' secrecy | **C, O:** a counting argument over the per-column openings only, not checked against a written theorem for Plonky3's hiding PCS. It does not cover the FRI commit-phase openings (see Z7) |
