...
Status: **draft, 2026-09-25; for the owner's approval.**
...
|---|---|---|
| **Incident lead** | The owner, or a deputy the owner names before launch | Declares incidents, decides halts, resets and public statements |
| **Developer on call** | Named before launch | Diagnosis, fixes, patched releases, AUDIT.md entry |
| **Operators** | Whoever runs each of the seven machines | Watch the signals (§3), preserve evidence (§5), apply instructions |

Before launch, fill in:
- names;
- a private operators' channel (not the public one: vulnerability details must not
  leak before a fix);
...
|---|---|---|---|
| **S1: critical** | Consensus, soundness or privacy failure | Nodes disagree on valid blocks; supply check fails; a proof that should fail verifies; a leak of private data | Immediate. Halt as in §4.1. Private until fixed |
...
**Steps:**
1. The incident lead tells operators, over the private channel, to **stop miners
   first**, then nodes.
...
These go to the incident lead. **Logs contain IP addresses of peers; treat them as
private.**
