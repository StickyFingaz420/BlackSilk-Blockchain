...
exports.RateLimitFactory = exports.RateLimiter = void 0;
class RateLimiter {
    ...
    async checkRateLimit(key) {
        ...
        await this.db.query('DELETE FROM rate_limits WHERE key = ? AND created_at < ?', [key, windowStart.toISOString()]);
        // Count current requests
        const result = await this.db.query('SELECT COUNT(*) as count FROM rate_limits WHERE key = ? AND created_at >= ?', [key, windowStart.toISOString()]);
    ...
    async recordRequest(key) {
        await this.db.query('INSERT INTO rate_limits (key, ip_address, created_at) VALUES (?, ?, CURRENT_TIMESTAMP)', [key, key.split(':')[0]] // Extract IP from key
        );
    }
    /**
     ...
     */
    async isBlocked(key) {
        const result = await this.db.query('SELECT block_until FROM rate_limit_blocks WHERE key = ? AND block_until > ?', [key, Date.now()]);
        if (result.length > 0) {
            return {
                blocked: true,
    ...
    async blockKey(key) {
        ...
        await this.db.query('INSERT OR REPLACE INTO rate_limit_blocks (key, block_until, created_at) VALUES (?, ?, CURRENT_TIMESTAMP)', [key, blockUntil]);
    }
    /**
    ...
    async cleanup() {
        ...
        await this.db.query('DELETE FROM rate_limits WHERE created_at < ?', [cutoff.toISOString()]);
        await this.db.query('DELETE FROM rate_limit_blocks WHERE block_until < ?', [Date.now()]);
